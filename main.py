...
def parse_datetime(text: str, timezone: str) -> datetime | None:
    ...
    text_date = re.search(rf"\b(\d{{1,2}})\s+({month_names})(?:\s+(\d{{4}}))?\b", clean_key(text))
    if text_date:
...
def calendar_link(booking: Booking, duration_minutes: int, timezone: str) -> str:
    ...
    if booking.guide:
        for label, key in [("Одежда", "Одежда"), ("Позы", "Позы"), ("Подготовка", "Подготовка")]:
...
def format_client_message(booking: Booking, calendar_url: str) -> str:
    studio = booking.studio
    guide = booking.guide or {}
    arrive = studio.get("За сколько минут прийти", "10").strip() or "10"
    ...
    lines = [
        ...
        f"Студия: {studio.get('Название студии', '').strip()}",
        f"Адрес: {studio.get('Адрес', '').strip()}",
        "",
        f"Пожалуйста, приходите за {arrive} минут до начала.",
    ]

    parking = studio.get("Парковка", "").strip()
    entrance = studio.get("Как зайти", "").strip()
    extra = studio.get("Дополнительно", "").strip()

    if parking:
        lines.append(f"Парковка: {parking}")
    if entrance:
        lines.append(f"Как зайти: {entrance}")
    if extra:
        lines.append(extra)
    ...
    lines.extend(
        [
            ...
            calendar_url,
            "",
            "Пожалуйста, проверьте дату и время на всякий случай.",
        ]
    )

    guide_lines = []
    for label, key in [("Одежда", "Одежда"), ("Позы", "Позы"), ("Подготовка", "Подготовка"), ("Дополнительно", "Дополнительно")]:
...
def parse_booking(text: str, studios: list[dict], guides: list[dict], timezone: str) -> Booking:
    return Booking(
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        "Напишите мне съемку одной фразой, например:\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    studios = context.application.bot_data["studios"]
    guides = context.application.bot_data["guides"]

    booking = parse_booking(update.message.text, studios, guides, config["timezone"])
    missing = missing_fields(booking)
    if missing:
        await update.message.reply_text(
            "Мне не хватило: " + ", ".join(missing) + ".\n\n"
            "Попробуйте так:\nАнна, 12 августа 14:00, студия M50, беременность"
        )
        return

    link = calendar_link(booking, config["duration_minutes"], config["timezone"])
    await update.message.reply_text("Готово, вот сообщение для клиента:\n\n" + format_client_message(booking, link))
...
def main() -> None:
    ...
    app.bot_data["config"] = {"timezone": timezone, "duration_minutes": duration_minutes}
